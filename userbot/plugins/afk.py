    ...
    if not USER_AFK:  # pylint:disable=E0602
        ...
        if reason:
            await borg.send_message(
                event.chat_id, f"📣 `Admins are not available at this time. Please try again later.`"
            )
        else:
            await borg.send_message(event.chat_id, f"**Jon Iriz is now AFK!**.")
...
async def on_afk(event):
    ...
    if USER_AFK and not (await event.get_sender()).bot:  # pylint:disable=E0602
        ...
        msg = None
        message_to_reply = (
            f"📣 `Admins are not available at this time. Please try again later.`"
            if reason
            else f"📣 `Admins are not available at this time. Please try again later.`"
        )
